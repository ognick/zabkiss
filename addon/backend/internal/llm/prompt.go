...
- status "reject": команда невыполнима (устройство недоступно, действие запрещено), actions: []
- status "clarify": запрос неоднозначен, в reason — уточняющий вопрос, actions: []
- используй только entity_id и сервисы из списка выше
- reply всегда заполнен и понятен пользователю`
